...
def _withings_measure(start: str, end: str) -> List[Dict[str, Any]]:
    ...
    for g in res.get("body", {}).get("measuregrps", []):
        ...
        for m in g.get("measures", []):
            ...
            scaled = v * (10 ** u)
            if t == 1:   entry["weight_kg"]        = round(scaled, 1)
            if t == 5:   entry["fat_free_mass_kg"] = round(scaled, 1)
            if t == 6:   entry["fat_pct"]          = round(scaled, 1)
            if t == 8:   entry["fat_mass_kg"]      = round(scaled, 1)
            if t == 76:  entry["muscle_mass_kg"]   = round(scaled, 1)
            if t == 77:  entry["hydration_kg"]     = round(scaled, 1)
            if t == 88:  entry["bone_mass_kg"]     = round(scaled, 1)
...
def health_withings(days: int = 30, end: str | None = None) -> Dict[str, Any]:
    ...
    for offset in range(days - 1, -1, -1):
        d = (date.fromisoformat(end) - timedelta(days=offset)).isoformat()
        m = meas_map.get(d, {})
        rows.append({"date": d, "weight_kg": m.get("weight_kg"), "fat_pct": m.get("fat_pct"), "fat_mass_kg": m.get("fat_mass_kg"), "fat_free_mass_kg": m.get("fat_free_mass_kg"), "muscle_mass_kg": m.get("muscle_mass_kg"), "hydration_kg": m.get("hydration_kg"), "bone_mass_kg": m.get("bone_mass_kg")})
